...
exports.createAppointment = async(req, res) => {
    try
    {
        ...
        if ( slotRet )
        {
            const appRet = await app.create({id: uuid.v4(), patientId, slotId, prescription: "NONE", status: "Scheduled"})
            if ( appRet )
            {
...
exports.viewAppointmentById = async (req, res) => {
    try
    {
        ...
        const appRet = await app.findAll({include: [{model: slot, required: true, include: [{model: doc, required: true}]}, {model: pat, required: true}], where: {id: appId}})
        if ( appRet )
